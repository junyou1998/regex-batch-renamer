  ...
  <div class="flex flex-col flex-1 min-w-96 h-full min-h-0 overflow-hidden bg-slate-50/90 dark:bg-slate-900/90 select-text">
    ...
    <div class="border-b border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md shrink-0 select-none">
      ...
      <div class="px-4 pb-2.5 pt-0.5 flex items-center justify-between gap-2">
        ...
        >
          <ClaudeIcon className="w-3 h-3 text-[#D97757] shrink-0" />
          <span
          ...
          <span class="text-slate-600 dark:text-slate-300 font-medium truncate">
            ...
            <template v-else-if="aiStore.status?.ready">
              Claude Code <span class="text-slate-400 font-normal">({{ aiStore.status.version || 'Ready' }})</span>
            </template>
            <template v-else>
              {{ $t('ai.statusNotReady') }}
            </template>
    ...
    >
      ...
      >
        <div class="flex items-start gap-2.5 text-amber-800 dark:text-amber-300">
          ...
          <div class="space-y-1 text-xs">
            <p class="font-semibold text-sm">{{ $t('ai.onboardingTitle') }}</p>
        ...
        <div class="space-y-2 pt-1 text-xs">
          <div class="space-y-1">
            <span class="font-medium text-slate-700 dark:text-slate-300">
              1. {{ $t('ai.stepInstallCli') }}
            </span>
            <div
              class="flex items-center justify-between gap-2 p-2 rounded-lg bg-slate-900 text-slate-100 font-mono text-[11px]"
            >
              <span class="truncate">npm i -g @anthropic-ai/claude-code</span>
              <button
                @click="copyToClipboard('npm i -g @anthropic-ai/claude-code', 'install')"
          ...
          <div class="space-y-1">
            ...
            <div
              class="flex items-center justify-between gap-2 p-2 rounded-lg bg-slate-900 text-slate-100 font-mono text-[11px]"
            >
              <span class="truncate">claude login</span>
              <button
                @click="copyToClipboard('claude login', 'login')"
                class="text-slate-400 hover:text-white transition-colors cursor-pointer shrink-0"
              >
      ...
      <template v-else>
        ...
        >
          ...
          <div
            v-if="msg.role === 'user'"
            class="max-w-[85%] rounded-2xl rounded-tr-xs bg-blue-600 text-white px-3.5 py-2.5 text-sm shadow-xs leading-relaxed"
          >
            {{ msg.content }}
          </div>
    ...
    >
      ...
      >
        <textarea
          ...
          class="w-full bg-transparent resize-none text-xs sm:text-sm text-slate-800 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none custom-scrollbar"
        ></textarea>
