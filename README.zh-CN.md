...
- **🕒 更名历史记录与日志审计**：
    - 自动持久化存储每次重命名、副本与撤销批量操作。
    - 可视化双栏时间轴、文件前后对照 Diff、关键词搜索与状态筛选。
    - 一键「载入当时规则管道」，快速复现过往规则。
    - 支持通过原生系统对话框导出 **CSV** 或 **JSON** 审计报表（支持单一聚焦批次或全局历史）。
- **📊 底部信息状态栏**：即时呈现文件数、文件夹数、变更笔数、冲突警告、作用范围切换（仅文件名/完整名称）、AI 连接状态与撤销快捷键。
