...
- **🕒 更名歷程紀錄與日誌審計**：
    - 自動持久化儲存每次更名、副本與復原批次操作。
    - 視覺化雙欄時間軸、檔案前後對照 Diff、關鍵字搜尋與狀態篩選。
    - 一鍵「載入當時規則管道」，快速復現過往規則。
    - 支援透過原生系統對話框匯出 **CSV** 或 **JSON** 審計報表（支援單一聚焦批次或全域歷史）。
- **📊 底部資訊狀態列**：即時呈現檔案數、資料夾數、變更筆數、衝突警告、作用範圍切換（僅檔名/完整名稱）、AI 連線狀態與復原快捷鍵。
